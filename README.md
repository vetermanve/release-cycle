...
жизненный цикл релизного поезда: feature -> dev -> test -> предпрод+прод (release) ->
accept (merge в master), плюс stop-the-line и конфликты. Всё на локальном GitLab + GitLab CI.
...
и деплоит **привязанные** стенды. Любой стенд можно собрать **напрямую**, минуя другие (починить test минуя dev).

- **Авто-рефреш** (пуш в `feature/bt-*` или смена `bt-set`) рефрешит **только активные dev/test**.
  prepod/prod меняются **только явным промоушном** (`make release`) — прод стабилен.
- **`make release`** катит prepod+prod, но **master НЕ трогает**.
- **`make accept`** (прод принят, ничего не упало): `prod -> master` + tag, затем master подмержен ->
  активные dev/test пересобираются от свежего master. Только так релиз попадает в master.
...
make release DATE=26.06.09               # prepod (8083) + prod (8084); master НЕ трогаем
make accept  DATE=26.06.09               # прод принят -> merge prod->master + tag -> рефреш dev/test
...
| `make release DATE=` | привязать prepod+prod -> собрать (master НЕ трогает) |
| `make accept DATE=` | прод принят: merge prod->master + tag + рефреш активных dev/test |
...
- **`stands/<env>`** (по файлу на стенд: `stands/dev|test|prepod|prod` — в каждом дата поезда) = desired state
  «какой поезд на стенде». `<env>-DATE = master + БТ поезда + merge-ветки` — симметрично для всех env.
- **Кто что рефрешит:**
  - смена `stands/<env>` (явный промоушн: `make dev/test/release`) -> ИМЕННО этот стенд (любой env);
  - пуш в `feature/bt-*` или смена `bt-set` -> **только активные dev/test** (prepod/prod не трогаются);
  - `make accept` -> merge `prod->master` + tag, затем рефреш активных dev/test (релиз попал в master).
- **prod при сборке master НЕ мержит** — это делает только `accept` после приёмки. Так поезда не утекают
  друг в друга через master, а прод не меняется от пуша в фичу.
- `seed/release-repo/.gitlab-ci.yml` — один джоб `reconcile` (changes `trains/**/bt-set.yaml` или `stands/**`,
  либо `ACTION=reconcile|accept`).
...
- **Rollback:** `git revert` коммита `stands/<env>` -> reconcile откатывает стенд на прошлый поезд.
  master в проде растёт только через `accept`, поэтому откат стенда не задевает релизную базу.
