...
make test      # автономные ассерты конечного состояния
...
## Ручной сценарий (для показа)

```bash
make create-train DATE=26.06.09 BTS=16,25   # собрать поезд -> dev-стенд (8081)
make create-train DATE=26.06.09 BTS=16,25,30 # добавить multi-repo БТ-30
make train DATE=26.06.09                     # отправление -> test-стенд (8082)
make gate  DATE=26.06.09 RESULT=pass         # предпрод (8083) -> прод (8084), merge в master + тег
make inject-defect DATE=26.06.11 BT=99       # stop-the-line: gate fail -> поезд остановлен + postmortem
```
...
| `make test` | ассерты конечного состояния |
| `make status` | поезда + стенды |
| `make create-train DATE= BTS=` | создать/обновить поезд (reconcile dev) |
| `make train DATE=` | отправление (cut test-) |
| `make gate DATE= RESULT=pass\|fail` | промоушн или stop-the-line |
