...
- **Тройные+ конфликты — вручную:** reconcile авто-создаёт только парные skeleton. Для кластера
  создай ветку с ВСЕМИ БТ в имени: `make mkmerge REPO=svc-a BTS=77,78,79` -> `merge/bt-77-bt-78-bt-79`.
  Подключается в релиз **автоматически по имени** (БТ⊆поезда). Subset-фильтр предпочитает больший
  кластер (pair ⊂ triple -> берётся triple), устаревшие пары можно удалить.
