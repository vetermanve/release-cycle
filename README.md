...
- **`stands.yaml`** (binding `стенд -> поезд`) меняется -> CI **promote**: довести ветку
  (`test-`/`release-`) и задеплоить test/prepod/prod. Промоушн = git-правка (UI или `make test/release`).
- `seed/release-repo/.gitlab-ci.yml` — джобы `reconcile` (changes `trains/**/bt-set.yaml`) и
  `promote` (changes `stands.yaml`).
...
- **Rollback:** `git revert` коммита `stands.yaml` -> CI откатывает стенд на прошлый поезд.
